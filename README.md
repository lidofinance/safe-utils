...
This repository contains a standalone build of the Safe Multisig Transaction Hashes web interface. The standalone build is a static version of the web interface that can be opened on local machines. It does not require any setup or dependencies.

To use the standalone build, download the latest release from the [releases](https://github.com/lidofinance/safe-utils/releases). Once unzipped, open the `index.html` file in your browser to use the Safe Multisig Transaction Hashes web interface. Release can be generated by running `npm run build:standalone` in the `app` directory.

Alternatively, you can use the hosted version of Safe Hash Preview at [https://safeutils.openzeppelin.com/](https://safeutils.openzeppelin.com/).
